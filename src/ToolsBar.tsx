...
import type { ChannelPreview } from "./core/image/ImageChannels";
...
const getChannelButtonClass = (channel: string, isActive: boolean) => {
  ...
  switch (channel) {
    case "gray":
      return `${baseClass} channels-bar__btn--gray ${activeClass}`;
    case "red":
      return `${baseClass} channels-bar__btn--red ${activeClass}`;
    case "green":
      return `${baseClass} channels-bar__btn--green ${activeClass}`;
    case "blue":
      return `${baseClass} channels-bar__btn--blue ${activeClass}`;
    case "alpha":
      return `${baseClass} channels-bar__btn--alpha ${activeClass}`;
    default:
      return baseClass;
  }
};
    ...
    <aside className="tools-bar">
      ...
      <div className="tools-bar__buttons">
        ...
        <button
          type="button"
          className="tools-bar__btn"
          onClick={onKernelsClick}
          disabled={!hasImage}
          title={
            hasImage
              ? "Фильтры (ядра свёртки, медианный и др.)"
              : "Сначала загрузите изображение"
          }
        >
          <span className="tool-icon">
            <img src="/filter-svgrepo-com.svg" alt="Фильтры" />
          </span>
        </button>
          ...
          <div className="channels-bar">
            ...
            {!isChannelsCollapsed && (
              <div className="channels-bar__buttons">
                {isGb7Image ? (
                  <button
                    onClick={() => onChannelToggle("grayscale")}
                    className={getChannelButtonClass(
                      "gray",
                      grayscaleActive
                    )}
                    title="Канал градаций серого"
                  >
                    <div className="channels-bar__btn-content">
                      {getChannelPreview("gray") && (
                        <img
                          src={getChannelPreview("gray")}
                          alt="Grayscale channel preview"
                          className="channel-preview"
                        />
                      )}
                      <span className="channel-label">
                        {getChannelLabel("gray")}
                      </span>
                    </div>
                  </button>
                ) : (
                ...
                {/* Альфа-канал - теперь с таким же превью, как у RGB */}
                {hasAlphaChannel && (
                  <button
                  ...
                  >
                    <div className="channels-bar__btn-content">
                      ...
                      <span className="channel-label">
                        ...
                        {!activeChannels.alpha && (
                          <span className="channel-status"> (скрыт)</span>
                        )}
                      </span>
                    </div>
                  </button>
              ...
              <div className="tools-bar__color-preview">
                ...
                <div className="color-info">
                  ...
                  >
                    ...
                    <span className="color-value">
                      L*{pixelInfo.labL} a*{pixelInfo.labA} b*{pixelInfo.labB}
                    </span>
                  </div>
                </div>
              </div>
